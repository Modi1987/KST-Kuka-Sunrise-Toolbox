...
if ~exist('t','var') || isempty(t)
  ...
  return;
      ...
      movePTPJointSpace( t , pos, relVel); % go to home position
      ...
      movePTPCirc1OrintationInter( t , f1,f2, vel)
...
Cen=getEEFPos(t); % Concider the current position as the center of the arcs
...
movePTPLineEefRelBase( t , Pos, vel);
%% Store the current position in the memory
circle_Starting_Point=getEEFPos( t ); % Consider the current point as circle starting point
...
movePTPArc_AC(t,theta,c,k,vel)
      
%% Go back to ((circle_Starting_Point)) coordinates
vel=150;
movePTPLineEEF( t , circle_Starting_Point, vel);
